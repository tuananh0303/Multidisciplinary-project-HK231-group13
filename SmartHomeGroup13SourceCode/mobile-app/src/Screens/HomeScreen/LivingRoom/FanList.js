...
const FanListLivingroom = ({ navigation }) => {
  ...
  useEffect(() => {
    ...
    if (deviceData) {
      setChecked(deviceData.fan); // Cập nhật trạng thái của Switch khi deviceData thay đổi
    }
  }, [deviceData]);

  const handleChange = (value) => {
    setChecked(value);
    handleControl("fan", value);
  };
    ...
    <View flexDirection="column">
      ...
      <View style={[styles.status, styles.icon]}>
        ...
        <TouchableOpacity onPress={onDoorListLivingroom}>
          <MaterialCommunityIcons name="door" color={"#bcbcbc"} size={50} />
        </TouchableOpacity>
      </View>
      <View style={styles.list}>
        <View flexDirection="row">
          <Text style={styles.listText}>Fan</Text>
          <Switch
